...
import { Curriculum } from '../../src/index'
...
class AddCurriculum extends Component {
    ...
    handleProgram = (text) => {
        ...
        if (text === 'BSCS') {
            majorOptions = ['Data Science'];
        } else if (text === 'BSIT') {
            majorOptions = ['System Development', 'Business Analytics'];
        }
    ...
    save = async () => {
        const { program, major, year, semester, courses } = this.state;

        if (!program || !major || !year || !semester || !Array.isArray(courses) || courses.length === 0) {
        ...
        try {
            ...
            // Map the fetched course details to include necessary properties for creation
            ...
            console.log('Courses for Curriculum:', coursesForCurriculum);

            // Save details with the associated courses
            await curriculum.create(program, major, year, semester, coursesForCurriculum);
                ...
                Alert.alert('Created Successfully', 'Details saved successfully.', [
                    {
                        ...
                        onPress: () => {
                            ...
                            this.props.navigation.navigate('CurriculumList');
                        },
                    }
                ]);
        ...
        <View style={{ 
            ...
            <MultiSelect
                ...
                onConfirm={() => {
                    ...
                    const { courses, program, major, year, semester} = this.state;
                    if (!program || !major || !year || !semester || courses.length === 0) {
                        Alert.alert('Input failed', 'Please fill in all fields and select courses.', [{ text: 'OK' }]);
                        return;
                    }

                    // Call the save function to handle saving the selected courses
                    this.save(this.state.program, this.state.major, this.state.year, this.state.semester, this.state.courses);
...
{/* save button */}
    <TouchableOpacity style={{
        ...
        onPress = {() => this.save(this.state.program, this.state.major, this.state.year, this.state.semester, this.state.courses)} 
        >
